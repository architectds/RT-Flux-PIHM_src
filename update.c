...
void update(realtype t, void *DS)
	{
  	...
  	MD = (Model_Data)DS;
    	...
    	for(k=0; k<MD->NumSdown; k++)
     		{                               
 		while(MD->TSD_Sdown[k].iCounter < MD->TSD_Sdown[k].length && t/(UNIT_C) > MD->TSD_Sdown[k].TS[MD->TSD_Sdown[k].iCounter+1][0])
     			{
      			MD->TSD_Sdown[k].iCounter++;
    			}
   		}
   	for(k=0; k<MD->NumLdown; k++)
     		{  
      		while(MD->TSD_Ldown[k].iCounter < MD->TSD_Ldown[k].length && t/(UNIT_C) > MD->TSD_Ldown[k].TS[MD->TSD_Ldown[k].iCounter+1][0])
          		{
          		MD->TSD_Ldown[k].iCounter++;
           		}
    		}
 	for(k=0; k<MD->NumP; k++)  
   		{     
