...
/* Lateral Flux Calculation between Triangular elements Follows  */
	for(i=0; i<MD->NumEle; i++)
		{
//		printf("\nELe:%d, ET=%f, %f, %f ",i,MD->EleET[i][0],MD->EleET[i][1],MD->EleET[i][2]);
...
//				Avg_Y_Sub=avgY(MD->Ele[i].zmin,MD->Ele[MD->Ele[i].nabr[j]-1].zmin,MD->DummyY[i+2*MD->NumEle],MD->DummyY[MD->Ele[i].nabr[j]-1 + 2*MD->NumEle]);
				...
				// Avg_Ksat= (Dif_Y_Sub>0 ? effK:effKnabr);  /* one point upwinding scheme for flux calculation */
        			/* groundwater flow modeled by Darcy's law */
        			MD->FluxSub[i][j] = Avg_Ksat*Grad_Y_Sub*Avg_Y_Sub*MD->Ele[i].edge[j]; 
				MD->VeloSub[i][j] = Avg_Ksat*Grad_Y_Sub;
				MD->DistSub[i][j] = Distance;
				MD->AreaSub[i][j] = Avg_Y_Sub*MD->Ele[i].edge[j];
				/***************************************************************************/
...
//            				Avg_Y_Sub = (MD->DummyY[i+2*MD->NumEle] + (Interpolation(&MD->TSD_EleBC[(MD->Ele[i].BC[j])-1], t) - MD->Ele[i].zmin))/2;
			...
			if (i==30 && MD->FluxSurf[i][0] + MD->FluxSurf[i][1] + MD->FluxSurf[i][2]!=0)
				{
//				printf("\n Ele %d surface flow = %f", i, MD->FluxSurf[i][0] + MD->FluxSurf[i][1] + MD->FluxSurf[i][2]);
				}
    			} 
		...
		if(MD->DummyY[i+2*MD->NumEle]>AquiferDepth-MD->Ele[i].infD)
			...
			if (MD->DummyY[i] + (MD->EleNetPrep[i] + (MD->FluxSurf[i][0] + MD->FluxSurf[i][1] + MD->FluxSurf[i][2])/MD->Ele[i].area - MD->EleViR[i] - (MD->DummyY[i]<(EPS/100)?0:MD->EleET[i][2]))/(UNIT_C)<0)
				{
				MD->EleViR[i] = MD->DummyY[i]*(UNIT_C) + MD->EleNetPrep[i]+ (MD->FluxSurf[i][0] + MD->FluxSurf[i][1] + MD->FluxSurf[i][2])/MD->Ele[i].area - (MD->DummyY[i]<(EPS/100)?0:MD->EleET[i][2]);
...
//			if (i==296) printf("\n sfc rate = %f", MD->DummyY[i]*UNIT_C + MD->EleNetPrep[i] + (MD->FluxSurf[i][0] + MD->FluxSurf[i][1] + MD->FluxSurf[i][2])/MD->Ele[i].area -  MD->EleViR[i] - (MD->DummyY[i]<EPS/100?0:MD->EleET[i][2]));
//			if (i==296) printf("\n after statement MD->DummyY[i] = %f, sfc flow = %f, net precip = %f, ViR = %f, ET = %f,", MD->DummyY[i]*UNIT_C, (MD->FluxSurf[i][0] + MD->FluxSurf[i][1] + MD->FluxSurf[i][2])/MD->Ele[i].area, MD->EleNetPrep[i], MD->EleViR[i], ((MD->DummyY[i]<EPS/100?0:MD->EleET[i][2])));
			if (MD->DummyY[i] + (MD->EleNetPrep[i] + (MD->FluxSurf[i][0] + MD->FluxSurf[i][1] + MD->FluxSurf[i][2])/MD->Ele[i].area - MD->EleViR[i] - (MD->DummyY[i]<EPS/100?0:MD->EleET[i][2]))/(UNIT_C)<0)
				{
				MD->EleViR[i] = MD->DummyY[i]*(UNIT_C) + MD->EleNetPrep[i]+ (MD->FluxSurf[i][0] + MD->FluxSurf[i][1] + MD->FluxSurf[i][2])/MD->Ele[i].area - (MD->DummyY[i]<EPS/100?0:MD->EleET[i][2]);
				MD->EleViR[i] = MD->EleViR[i]<0?0:MD->EleViR[i];
				}
//			if (i==296) printf("\n before MD->DummyY[i] = %f, sfc flow = %f, net precip = %f, ViR = %f, ET = %f,", MD->DummyY[i], (MD->FluxSurf[i][0] + MD->FluxSurf[i][1] + MD->FluxSurf[i][2])/MD->Ele[i].area, MD->EleNetPrep[i], MD->EleViR[i], ((MD->DummyY[i]<EPS/100)?0:MD->EleET[i][2]));

//			if (i==30) printf("\n f.c infiltration = %f, effK = %f, Grad_Y_Sub = %f", MD->EleViR[i], effK, Grad_Y_Sub);
//			if (i==30) printf("\n f.c infiltration = %f", MD->EleViR[i]);
...
//		if (i==296) printf("\n MD->DummyY[i] = %f, sfc flow = %f, net precip = %f, ViR = %f, ET = %f,", MD->DummyY[i], (MD->FluxSurf[i][0] + MD->FluxSurf[i][1] + MD->FluxSurf[i][2])/MD->Ele[i].area, MD->EleNetPrep[i], MD->EleViR[i], ((MD->DummyY[i]<EPS/100)?0:MD->EleET[i][2]));
