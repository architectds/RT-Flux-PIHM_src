  ...
  *rawtime = (int) (t * 60);
  ...
  for (i = CD->NumEle; i < CD->NumEle * 2; i++)
	...
	* MD->Ele[i - MD->NumEle].area * CD->Vcele[i].porosity;
      ...
      CD->Vcele[i].q += fabs (MD->EleET[i - MD->NumEle][2])
	    ...
	    Chem_Data CD, realtype t)
{
  ...
  for (i = 0 ; i < CD->NumPUMP; i ++)
    {
      CD->pumps[i].flow_rate = CD->pumps[i].flow_rate / total_area / UNIT_C;
      fprintf (stderr, " -- Flow rate is then %g meter per minute\n",
	       CD->pumps[i].flow_rate);
    }
	...
	if (MD->Ele[i].nabr[j] > 0)
	  {
	    ...
	    CD->Flux[k].distuu   = (MD->Ele[MD->Ele[i].nabr[j] -1].x - MD->Ele[i].x) / CD->Flux[k].distance;
	    // x component of the normal vector flux direction
	    CD->Flux[k].distll   = (MD->Ele[MD->Ele[i].nabr[j] -1].y - MD->Ele[i].y) / CD->Flux[k].distance;
	    // y component of the normal vector flux direction

	    //	    fprintf(stderr, " %f \t", CD->Flux[k].distuu* CD->Flux[k].distuu + CD->Flux[k].distll * CD->Flux[k].distll);
  ...
  CD->NumLateral = k;
  fprintf(stderr, "Number of total lateral interflow faces: %d\n", CD->NumLateral);
  for (i = 0; i < MD->NumEle; i++)
    {
      for (j = 0; j < 3; j++)
	if (MD->Ele[i].nabr[j] > 0)
      ...
      for (j = 0; j < CD->NumStc; j++)
	{
	  ...
	  if ((CD->chemtype[j].itype == 3) || (CD->chemtype[j].itype == 2))
		...
		CD->Vcele[i].t_conc[j] * (1 - CD->Vcele[i].porosity) * 2650;
	      CD->Vcele[i].p_conc[j] = CD->Vcele[i].t_conc[j];
	      //	      fprintf (stderr,
	      //       "site concentration is %f mole per porous space L\n",
	      //	       CD->Vcele[i].t_conc[j]);
  ...
  *rawtime = (int) (t * 60);
  ...
  for (i = 0; i < MD->NumRiv; i++)
    {
      ...
      CD->Flux[k].flux += MD->FluxRiv[i][2];
      k++;
    }
  for (i = 0; i < MD->NumRiv; i++)
    {
      //    CD->Flux[k].flux   += MIN(MD->FluxRiv[i][3],0.0);
      CD->Flux[k].flux += MD->FluxRiv[i][3];
      k++;
    }
  /*
     for ( i = 0; i <MD->NumRiv; i++){
  ...
  if (((int) timelps  -(int) CD->TimLst) == CD->AvgScl * stepsize)
      ...
      for (k = 0; k < CD->NumFac; k++)
	{
	  ...
	  CD->Vcele[CD->Flux[k].nodeup-1 + CD->NumEle].height_tl += CD->Flux[k].velocity * CD->Flux[k].distll;
      ...
      for (i = 2 * MD->NumEle; i < 2 * MD->NumEle + MD->NumRiv; i++)
	{
	  ...
	  CD->Vcele[i].area =
	    MD->Riv[j].Length *
	    CS_AreaOrPerem (MD->Riv_Shape[MD->Riv[j].shape - 1].interpOrd,
			    Y[j + 3 * MD->NumEle], MD->Riv[j].coeff, 3);
	  CD->Vcele[i].vol_o = CD->Vcele[i].area * CD->Vcele[i].height_o;
	  CD->Vcele[i].vol = CD->Vcele[i].area * CD->Vcele[i].height_t;
	}
      /* update EBR cells */
      for (i = 2 * MD->NumEle + MD->NumRiv; i < 2 * (MD->NumEle + MD->NumRiv);
	   i++)
	{
	  j = i - 2 * MD->NumEle - MD->NumRiv;
	  CD->Vcele[i].height_o = CD->Vcele[i].height_t;
	  CD->Vcele[i].height_t =
	    MAX (Y[i + MD->NumEle],
		 1.0E-5) + MAX (Y[i + MD->NumEle - MD->NumRiv],
...
InitialChemFile (char *filename, int NumBTC, int *BTC_loc)
{
  ...
  cfn[3+NumBTC] = (char *) malloc ((strlen (filename) + 20) * sizeof (char));
  sprintf (cfn[3+NumBTC], "output/%s.vol", filename); 
  ...
  *rawtime = (int) (t * 60);
  ...
  if ((int) timelps % (CD->OutItv * 60) == 0)
      ...
      for (i = 3; i < 3 + CD->NumBTC; i++)
	{
      ...
      cfn[3+CD->NumBTC] = (char *) malloc ((strlen (filename) + 12) * sizeof (char));
      sprintf (cfn[3+CD->NumBTC], "output/%s.vol", filename);
	  ...
	  double *start_step, int num_blocks)
{
      ...
      for (i = 0; i < CD->NumPUMP; i++)
	   ...
	   for ( j = 0 ; j < CD->NumEle; j ++)
		 ...
		 CD->pumps[i].flow_rate * CD->pumps[i].Injection_conc * stepsize * CD->Vcele[i].area / (CD->Vcele[j].porosity *
							   CD->Vcele[j].vol);
	       /*
	       if (j == 1) fprintf (stderr, " m_inverse %6.4f, ainj %6.4g, add %6.4g, time %6.4f\n",
  ...
  if ((!CD->RecFlg)
    ...
    {
      fprintf (stderr,
	       "  React from %f to %f, Average newton iteration taken: %f, time elapsed %6.4f seconds\n",
	       timelps - (CD->React_delay * stepsize), timelps,
	       (double) tot_nr / (double) CD->NumEle / 2.0, timer2 - timer1);
    }
}
